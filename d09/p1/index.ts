...
for (const seq of inputs) {
  ...
  while (stack.length > 1) {
    // Pop the next sequence and calculate the next element using the
    // delta from the last sequence
    const lastSeq = stack.pop()!
    delta = lastSeq[lastSeq.length - 1] + delta
  }

  // Now we know the delta for the original sequence's next element
  sum += seq[seq.length - 1] + delta
}
