...
type Position = [number, number, Direction, number]
...
const path = aStarSearch({
  ...
  getNeighbours: (key) => {
    ...
    // If we have done the minimum number of forward steps, we can turn
    if (forwardCounter >= MIN_FORWARD_STEPS) {
      if (prevDirection === 'u' || prevDirection === 'd') {
        if (x >= MIN_FORWARD_STEPS) {
          neighbours.push(posToKey([x - 1, y, 'l', 1]))
        }
        if (x <= maxX - MIN_FORWARD_STEPS) {
          neighbours.push(posToKey([x + 1, y, 'r', 1]))
        }
      } else {
        if (y >= MIN_FORWARD_STEPS) {
          neighbours.push(posToKey([x, y - 1, 'u', 1]))
        }
        if (y <= maxY - MIN_FORWARD_STEPS) {
